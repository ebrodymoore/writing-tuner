...
export function formatGuideUpdatePrompt(currentGuide, annotations, outputType = null) {
  ...
  const outputTypeInstruction = outputType
    ? `\n10. Place structure and length preferences specific to "${outputType}" in the "Output-Type Rules: ${outputType}" section rather than general Structure Preferences`
    : '';
...
11. Keep the guide under ~4000 tokens. Merge redundant rules. High-confidence preferences get examples; low-confidence get table entries only
12. Maintain the exact markdown format of the current guide (same sections, same table format)
13. Return ONLY the updated guide markdown, nothing else`;
}
